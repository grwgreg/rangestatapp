  ...
  return {
    ...
    link: function(scope, element, attrs, rangeStringDummyCtrl) {
      ...
      scope.$watch('preflopHands', function() {
        ...
        oldval = _.map(oldval, function(el) {
          return el.trim();
        });
        newval = _.map(newval, function(el) {
          return el.trim();
        });
      ...
      function stringWatcher() {
        ...
        try {
          ...
          if (err === 'invalid tag') { //todo explicitly handle bad input instead of throwing errors
            success = false;
      ...
      scope.$watch(function() {
        return rangeStringDummyCtrl.$viewValue;
      }, stringWatcher);
    }
  }
}]);
